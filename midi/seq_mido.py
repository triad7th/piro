...
def _test():
    ...
    # import midifile
    mid = MidiFile('.\\source\\mido test\\midi\\furelise.mid')
    mid_ppqn = mid.ticks_per_beat
    mid_bpm = 120
    for msg in mid:
        if msg.is_meta and msg.type=='set_tempo':
            mid_bpm = mido.tempo2bpm(msg.tempo)
            break
    
    print (mid_bpm, mid_ppqn)        
    seq = SequencerThread(midiout, bpm=mid_bpm, ppqn=mid_ppqn)

    tick = 0
    for msg in mid.tracks[1]:
        if not msg.is_meta:
            if msg.type == 'note_on':
                seq.add(msg.bytes(), tick)
                tick += msg.time
                seq.add((NOTE_OFF, msg.note, 0), tick)
